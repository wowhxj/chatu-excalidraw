...
;; Optional integration: when the separate `chatu-ai' package is
;; installed, `chatu-excalidraw-open' routes through it to offer AI
;; generation/modification. chatu-excalidraw does not depend on it —
;; without it, opening just creates an empty diagram, as before.
(declare-function chatu-ai-maybe-generate "chatu-ai")
...
If the optional `chatu-ai' package is installed, opening a
not-yet-existing .excalidraw file first prompts for an AI description
and can generate the diagram (and \\[universal-argument] on an
existing file offers AI modification); see `chatu-ai-maybe-generate'.
Without `chatu-ai', a missing file is created empty, as before."
  (interactive)
  (let* ((input-path (plist-get keyword-plist :input-path))
    ...
    (if (require 'chatu-ai nil t)
        (chatu-ai-maybe-generate
      ...
      (unless (file-exists-p input-path)
